...
*   **功能**: 将原始 PEMS 数据转换为模型可用的张量格式，集成缺失值修复、多周期采样和特征工程。
*   **核心逻辑**:
    1.  **加载与修复**: 读取 `.npz` 文件，使用线性插值修复缺失值，并基于训练集进行 Z-Score 归一化。
    2.  **构建邻接矩阵**: 根据 `.csv` 中的距离信息，使用高斯核函数计算节点间的语义相似度矩阵。
    3.  **特征工程 (Time Embedding)**: 提取时间戳的 "Hour of Day" 和 "Day of Week" 特征并归一化，拼接到原始特征（流量、占有率、速度）后，形成 5 通道输入。
    4.  **多周期采样 (Multi-period Sampling)**: 
        *   实现了 `Week/Day/Recent` 三种周期的历史数据采样。
        *   根据配置（如 `0周 1日 1最近`）提取对应的时间片段并拼接。
    5.  **数据集生成**: 生成包含 `(T, N, F_in)` 格式的训练/验证/测试集，其中 `T` 为拼接后的总时间步长。
    6.  **保存**: 输出 `train_data.npz`、`adj_mat.npy` 和 `scaler_params.json`。
